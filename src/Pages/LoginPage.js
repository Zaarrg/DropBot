...
async function Login() {
    ...
    await puppeteer.launch({ headless: (data.displayless) , executablePath: data.settings.Chromeexe, userDataDir: data.settings.UserDataPath, args: ['--no-sandbox']}).then(async browser => {
        ...
        if (data.debug && displayless) {
            ...
            console.log("DEBUG: Status Screen of Loginpage")
        }
        try {
